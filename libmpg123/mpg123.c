...
unsigned int mpg123_get_song_time(FILE * file)
{
	uint32_t head;
	...
	if (mpg123_decode_header(&frm, head))
	{
		...
		{
			free(buf);
			return ((unsigned int) (tpf * xing_header.frames * 1000));
		}
		free(buf);
		...
		fread(tmp, 1, 3, file);
		if (!strncmp((char *)tmp, "TAG", 3))
			len -= 128;
		return ((unsigned int) ((unsigned int)(len / bpf) * tpf * 1000));
	}
	return 0;
}
